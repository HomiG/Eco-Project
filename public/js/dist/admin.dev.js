...
var dateArr = [];
dateArr.push(fromDate, untilDate, fromMonth, untilMonth, fromDay, untilDay, fromTime, untilTime); // ----- FRONT ENT FUNCITON ----- 
...
function noCheckboxChoosed() {
  if (document.getElementById('IN_VEHICLE').checked == true || document.getElementById('ON_BICYCLE').checked == true || document.getElementById('ON_FOOT').checked == true || document.getElementById('STILL').checked == true || document.getElementById('TILTING').checked == true || document.getElementById('UNKNOWN').checked == true) {
    return false;
  } else {
    return true;
  }
...
var heatmapLayer = new HeatmapOverlay(cfg); // Declaer the baseLayer
...
var marker = L.marker(patrasLatLng).addTo(map); //Patras Circle Range
...
function rangedHeatmap() {
  ...
  if (noDateChoosed(dateArr) && noCheckboxChoosed()) {
    ...
    // YES DATE, NO CHEKCBOX
    mode = 2;
  } else if (noDateChoosed(dateArr) && !noCheckboxChoosed()) {
    //NO DATE, YES CHECKBOX
    mode = 3;
  } else if (!noDateChoosed(dateArr) && !noCheckboxChoosed()) {
    //YES DATE, YES CHECKBOX
    mode = 4;
  }
...
var btn = document.getElementById("delete_btn"); // Get the <span> element that closes the modal
...
var backbtn = document.getElementById("goback"); // When the user clicks on <span> (x), close the modal
